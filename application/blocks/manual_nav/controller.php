<?php
namespace Application\Block\ManualNav;
...
{
    protected $btTable = 'btManualNavigation';
    protected $btExportTables = ['btManualNavigation', 'btManualNavigationEntries'];
...
//    protected $btExportFileColumns = array('fID');
    ...
    {
        return t('Navigation block for when you don\'t want your nav to be generated from your site map.');
    }

    public function getBlockTypeName()
    {
        return t('Manual Navigation');
    }
    ...
    public function getSearchableContent()
    {
        ...
        $v = [$this->bID];
        $q = 'SELECT * from btManualNavigationEntries where bID = ?';
        $r = $db->query($q, $v);
        foreach ($r as $row) {
            $content .= $row['title'] . ' ';
        }
    ...
    {
        $db = $this->app->make('database')->connection();
        $query = $db->fetchAll('SELECT * from btManualNavigationEntries WHERE bID = ? ORDER BY sortOrder', [$this->bID]);
        $this->set('rows', $query);
    }
    ...
    {
        $db = $this->app->make('database')->connection();
        $r = $db->fetchAll('SELECT * from btManualNavigationEntries WHERE bID = ? ORDER BY sortOrder', [$this->bID]);
    ...
    public function duplicate($newBID)
    {
        ...
        $r = $db->query($q, $v);
        while ($row = $r->fetch()) {
            $db->executeQuery('INSERT INTO btManualNavigationEntries (bID, fID, icon, linkURL, title, sortOrder, internalLinkCID, internalLinkFID, openInNewWindow) values(?,?,?,?,?,?,?,?,?)', [
                $newBID,
                $row['fID'],
                $row['icon'],
                $row['linkURL'],
    ...
    {
        $db = $this->app->make('database')->connection();
        $db->delete('btManualNavigationEntries', ['bID' => $this->bID]);
        parent::delete();
    }
    ...
    {
        $db = $this->app->make('database')->connection();
        $db->executeQuery('DELETE from btManualNavigationEntries WHERE bID = ?', [$this->bID]);
        ...
        while ($i < $count) {
            ...
            $db->executeQuery('INSERT INTO btManualNavigationEntries (bID, fID, icon, title, sortOrder, linkURL, internalLinkCID, internalLinkFID, openInNewWindow) values(?,?,?,?,?,?,?,?,?)', [
                $this->bID,
                h($args['fID'][$i]),
                h($args['icon'][$i]),
                h($args['title'][$i]),
